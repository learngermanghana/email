...
with tabs[6]:
    st.markdown(
        """
        <div style='background:#e3f2fd;padding:1.2em;border-radius:12px;margin-bottom:1em'>
    ...
    raw_schedule_a2 = [
        ...
        ("Woche 5", ["5.12. Ein Tag im Leben (Übung)", "5.13. Ein Vorstellungsgespräch (Exercise)", "5.14. Beruf und Karriere (Exercise)"]),
    ...
    raw_schedule_b1 = [
        ("Woche 1", ["1.1. Traumwelten (Übung)", "1.2. Freundschaften fürs Leben (Übung)", "1.3. Erfolgsgeschichten (Übung)"]),
    ...
    selected_level = st.selectbox("🗂️ Kursniveau (A1/A2/B1)", list(course_levels.keys()), key="cs_level")
    topic_structure = course_levels[selected_level]
    st.markdown("---")

    col1, col2 = st.columns([2,1])
    with col1:
        start_date = st.date_input("Kursstart", value=date.today(), key="cs_start")
        holiday_dates = st.date_input("Ferien oder Feiertage (Mehrfachauswahl)", [], key="cs_holidays")
    with col2:
        advanced_mode = st.toggle("⚙️ Erweiterter Wochenmodus", value=False, key="cs_advanced")
    st.markdown("---")
    ...
    if not advanced_mode:
        days_per_week = st.multiselect("📌 Unterrichtstage wählen", options=days_of_week, default=default_days, key="cs_days_simple")
        for _, sessions in topic_structure:
            week_patterns.append((len(sessions), days_per_week or default_days))
    else:
        st.info("Für jede Woche individuelle Unterrichtstage festlegen")
        for i, (week_label, sessions) in enumerate(topic_structure):
            with st.expander(week_label, expanded=True):
                week_days = st.multiselect(week_label, options=days_of_week, default=default_days, key=f"cs_days_{i}")
                week_patterns.append((len(sessions), week_days or default_days))
    st.markdown("---")

    session_labels = [(wl, topic) for wl, topics in topic_structure for topic in topics]
    dates = []
    cur = start_date
    for num_sessions, week_days in week_patterns:
        week_dates = []
        while len(week_dates) < num_sessions:
            if cur.strftime("%A") in week_days and cur not in holiday_dates:
                week_dates.append(cur)
            cur += timedelta(days=1)
        dates.extend(week_dates)

    total_sessions = sum(num for num, _ in week_patterns)
    if len(dates) < total_sessions:
        st.error("⚠️ Nicht genug Unterrichtstage gefunden. Bitte passen Sie Ferien oder Modus an.")

    rows = [{"Week": wl, "Day": f"Day {i+1}", "Date": d.strftime("%A, %d %B %Y"), "Topic": tp}
            for i, ((wl, tp), d) in enumerate(zip(session_labels, dates))]
    df_schedule = pd.DataFrame(rows)
    st.markdown(f"""
    <div style='background:#fffde7;border:1px solid #ffe082;border-radius:10px;padding:1em;'>
      <b>📝 Kursüberblick:</b>
      <ul>
        <li><b>Kurs:</b> {selected_level}</li>
        <li><b>Start:</b> {start_date.strftime('%d.%m.%Y')}</li>
...
".join(f"- {r['Day']} ({r['Date']}): {r['Topic']}" for r in rows)
    ...
    class ColorHeaderPDF(FPDF):
        def header(self):
            ...
            self.cell(0,12, safe_pdf("Learn Language Education Academy - Course Schedule"), ln=True, align='C', fill=True)
            self.ln(2)
    ...
    # Header info
    for line in [f"Schedule: {selected_level}", f"Start: {start_date.strftime('%Y-%m-%d')}
"]:
        for sub in wrap_text(safe_pdf(line)):
            try:
    ...
    # Holidays
    if holiday_dates:
        holidays_line = "Holidays: " + ", ".join(d.strftime('%d.%m.%Y') for d in holiday_dates)
        for sub in wrap_text(safe_pdf(holidays_line)):
            try:
                pdf.multi_cell(0,8, sub)
            except Exception:
                pass
    ...
    # Schedule rows
    for r in rows:
        row_line = f"{r['Day']} ({r['Date']}): {r['Topic']}"
        for sub in wrap_text(safe_pdf(row_line)):
            try:
                pdf.multi_cell(0,8, sub)
            except Exception:
                continue
    ...
    pdf.cell(0,10, safe_pdf("Answered: Felix Asadu"), ln=True, align='R')

    out = pdf.output(dest='S')
    pdf_bytes = out.encode('latin-1','replace') if isinstance(out,str) else out
    st.download_button("📄 PDF Download", data=pdf_bytes, file_name=f"{file_prefix}.pdf", mime="application/pdf")
