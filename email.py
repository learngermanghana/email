...
# ---- Module-level schedule templates ----
RAW_SCHEDULE_A1 = [
...
with tabs[6]:
    ...
    import textwrap
    def break_long_words(line, max_len=40):
        return ' '.join(textwrap.wrap(line, width=max_len, break_long_words=True))

    def safe_for_fpdf(line):
        txt = line.strip()
        return not (len(txt) < 2 or (len(txt) == 1 and not txt.isalnum()))
    ...
    if holiday_dates:
        pdf.multi_cell(0,8, break_long_words(safe_pdf("Holidays: " + ", ".join(d.strftime('%d.%m.%Y') for d in holiday_dates))))
    pdf.ln(2)
    for r in rows:
        pdf.multi_cell(0,8, break_long_words(safe_pdf(f"{r['Day']} ({r['Date']}): {r['Topic']}")))
    pdf.ln(6)
