...
# ===== Helper: Make all PDF text safe (no Unicode crash) =====
def safe_pdf(text):
    try:
        # Remove/replace common unicode characters with latin equivalents
        replacements = {
            "ä": "ae", "ö": "oe", "ü": "ue", "ß": "ss",
            "Ä": "Ae", "Ö": "Oe", "Ü": "Ue",
            "é": "e", "è": "e", "à": "a", "ô": "o", "ê": "e"
...
def normalize_cols(df):
    df.columns = [
        c.strip().lower().replace(' ', '').replace('-', '').replace('/', '').replace('(', '').replace(')', '')
        for c in df.columns
    ]
    return df

def build_report_pdf(student_info, scores, ref_answers, school_name, assignment_total):
    pdf = FPDF()
    pdf.add_page()
    pdf.set_font('Arial', 'B', 14)
    pdf.cell(0, 10, safe_pdf(school_name), ln=True, align='C')
    pdf.set_font('Arial', 'B', 12)
    pdf.cell(0, 10, safe_pdf(f"Assignment Report for {student_info['name']} ({student_info['studentcode']})"), ln=True)
    pdf.ln(4)
    pdf.set_font('Arial', '', 11)
    pdf.cell(0, 8, safe_pdf(f"Level: {student_info.get('level','')}"), ln=True)
    pdf.cell(0, 8, safe_pdf(f"Email: {student_info.get('email','')}"), ln=True)
    pdf.cell(0, 8, safe_pdf(f"Date: {student_info.get('date', date.today())}"), ln=True)
    pdf.ln(2)
    completed = scores['assignment'].nunique()
    pdf.cell(0, 8, safe_pdf(f"Assignments: {assignment_total} | Completed: {completed} | Not Completed: {assignment_total-completed}"), ln=True)
    pdf.ln(4)
    ...
    for _, r in scores.iterrows():
        ...
        pdf.multi_cell(0, 7, safe_pdf(f"Comments: {str(r['comments'])}"))
        ref = ref_answers.get(r['assignment'])
        if ref:
            pdf.set_font('Arial', 'I', 9)
            pdf.multi_cell(0, 6, safe_pdf("Ref. Answers: " + " | ".join(ref)))
        pdf.ln(2)

    # Incomplete assignments
    done_assignments = set(scores['assignment'])
    for assignment, ref in ref_answers.items():
        if assignment not in done_assignments:
            pdf.set_font('Arial', 'B', 11)
            pdf.cell(0, 8, safe_pdf(f"{assignment}: Not Completed"), ln=True)
            if ref:
                pdf.set_font('Arial', 'I', 9)
                pdf.multi_cell(0, 6, safe_pdf("Ref. Answers: " + " | ".join(ref)))
            pdf.ln(2)
    ...
    pdf.cell(0, 10, safe_pdf(f"Generated by {school_name}"), ln=True, align='R')
    return pdf.output(dest='S')

# ===== Helper: Send PDF by email using SendGrid (if needed) =====
...
# ===== Marking Tab =====
with tabs[9]:
    st.title("📝 Assignment Marking & Scores (with Email, Reference Answers, & PDF Report)")
    ...
    # --- Assignment List ---
    @st.cache_data(show_spinner=False)
    def get_all_assignments(df_scores, ref_answers):
        assignments = set(df_scores['assignment'].dropna().unique())
        assignments |= set(ref_answers.keys())
        return sorted(assignments)
    all_assignments = get_all_assignments(df_scores, ref_answers)

    # ======== Classic Marking Mode ========
    st.header("Classic Mode: Mark One Assignment")
    search_name = st.text_input("Search student by name or code")
    sel_level = st.selectbox("Filter by Level", ["All"] + sorted(df_students['level'].dropna().unique().tolist()))
    ...
    student_row = students[students['studentcode'] == code].iloc[0]
    level = str(student_row.get('level', 'A1')).upper()
    assignment_total = LEVEL_TOTALS.get(level, 30)
    ...
    # ======== Batch Marking Mode ========
    st.header("Batch Mode: Enter All Assignments")
    batch_sel_level = st.selectbox("Select Level", sorted(df_students['level'].dropna().unique()), key="batch_level")
    batch_students = df_students[df_students['level'] == batch_sel_level]
    batch_search = st.text_input("Search student by name or code (Batch)", key="batch_search")
    if batch_search:
        batch_students = batch_students[
            batch_students['name'].str.lower().str.contains(batch_search.lower()) |
            batch_students['studentcode'].astype(str).str.lower().str.contains(batch_search.lower())
        ]
    batch_student_list = (batch_students['name'] + " (" + batch_students['studentcode'] + ")").unique()
    if len(batch_student_list):
        batch_chosen = st.selectbox("Select Student (Batch)", batch_student_list, key="batch_student")
        batch_code = batch_chosen.split('(')[-1].strip(')')
        batch_row = batch_students[batch_students['studentcode'] == batch_code].iloc[0]
        level_assignments = [
            a for a in all_assignments
            if batch_sel_level.lower() in a.lower()
            or a.lower().startswith(batch_sel_level.lower() + " ")
            or (batch_sel_level.lower() == "a1" and a.lower().startswith("lesen und horen"))
        ]
        stu_scores = df_scores[df_scores['studentcode'] == batch_code]
        batch_scores = {}
        with st.form(key="batch_form"):
            ...
            if submitted:
                ...
                for a, val in batch_scores.items():
                    df_scores = df_scores[~((df_scores['studentcode'] == batch_code) & (df_scores['assignment'] == a))]
                    newrow = {
                        'studentcode': batch_code,
                        'name': batch_row['name'],
                        'assignment': a,
                        'score': val,
                        'comments': '',
                        'date': now,
                        'level': batch_row.get('level','')
                    }
                    df_scores = pd.concat([df_scores, pd.DataFrame([newrow])], ignore_index=True)
                sync_scores_to_sqlite(df_scores)
                ...
                st.experimental_rerun()
        # Show batch history
        batch_hist = df_scores[(df_scores['studentcode'] == batch_code) & (df_scores['assignment'].isin(level_assignments))].sort_values('assignment')
        st.markdown("#### Entered Batch Scores")
        if not batch_hist.empty:
            st.dataframe(batch_hist[['assignment', 'score']])
        else:
            st.info("No batch scores entered yet for this student.")

    # ======= Score History and Reference Answers Table =======
    st.header("📄 Score History & Reference Answers")
    stu_scores = df_scores[df_scores['studentcode'] == code].sort_values('date', ascending=False)
    completed_assignments = stu_scores['assignment'].nunique()
    st.write(f"**Assignments completed: {completed_assignments} / {assignment_total}**")
    if not stu_scores.empty:
        show_df = stu_scores[['assignment','score','comments','date']].copy()
        show_df['reference_answer'] = show_df['assignment'].apply(lambda a: "; ".join(ref_answers.get(a, [])))
        st.dataframe(show_df, use_container_width=True)
    else:
        st.info("No scores found for this student.")

    # ======= PDF & EMAIL =======
    pdf_bytes = build_report_pdf(student_row, stu_scores, ref_answers, "Learn Language Education Academy", assignment_total)
    st.download_button(
        "📄 Download Full Report PDF",
        pdf_bytes,
        file_name=f"{student_row['name'].replace(' ','_')}_assignment_report.pdf",
        mime="application/pdf"
    )
    # Email button
        ...
        if st.button(f"📧 Email PDF to {student_email}"):
            try:
                ...
                message = Mail(
                    ...
                    to_emails=student_email,
                    subject="Your Assignment Results – Learn Language Education Academy",
                    html_content=f"<p>Hello {student_row['name']},<br><br>Your full assignment report is attached.</p>"
                )
                encoded = base64.b64encode(pdf_bytes).decode()
                attached = Attachment(
                    FileContent(encoded),
                    FileName(f"{student_row['name'].replace(' ','_')}_assignment_report.pdf"),
                    FileType('application/pdf'),
                    Disposition('attachment')
                )
                message.attachment = attached
