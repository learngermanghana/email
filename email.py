...
# ==== Helper Functions ====
def safe_pdf(text):
    # Remove/replace common unicode chars with ASCII/Latin-1 equivalents
    try:
        replacements = {
            "ä": "ae", "ö": "oe", "ü": "ue", "ß": "ss",
            "Ä": "Ae", "Ö": "Oe", "Ü": "Ue",
...
def build_report_pdf(student_info, scores, ref_answers, school_name, assignment_total):
    ...
    for _, r in scores.iterrows():
        ...
        if ref:
            pdf.set_font('Arial', 'I', 9)
            for ans in ref:
                pdf.multi_cell(0, 6, safe_pdf("Ref. Answer: " + ans))
        pdf.ln(2)
    # Incomplete assignments (with reference)
    done_assignments = set(scores['assignment'])
    for assignment, ref in ref_answers.items():
        if assignment not in done_assignments:
            pdf.set_font('Arial', 'B', 11)
...
with tabs[9]:
    ...
    # === Mode Selection ===
    mode = st.radio("Select Marking Mode", ["Classic (Mark One Assignment)", "Batch (All Assignments for Student)"], horizontal=True)
    ...
    if mode.startswith("Classic"):
        ...
        # Reference Answers
        if selected_assignment in ref_answers:
            st.markdown("**Reference Answers:**")
            for ans in ref_answers[selected_assignment]:
                st.write(ans)
        ...
        st.markdown("---")
        st.subheader("Batch: Mark All Assignments for This Student")
        level_assignments = [
            a for a in all_assignments
            if level.lower() in a.lower()
            or a.lower().startswith(level.lower() + " ")
            or (level.lower() == "a1" and a.lower().startswith("lesen und horen"))
        ]
