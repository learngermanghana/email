...
def build_report_pdf(student_info, scores, ref_answers, school_name, assignment_total):
    ...
    pdf.cell(0, 10, to_ascii(school_name), ln=True, align='C')
    pdf.set_font('Arial', 'B', 12)
    pdf.cell(0, 10, to_ascii(f"Assignment Report for {student_info['name']} ({student_info['studentcode']})"), ln=True)
    pdf.ln(4)
    pdf.set_font('Arial', '', 11)
    pdf.cell(0, 8, to_ascii(f"Level: {student_info.get('level','')}"), ln=True)
    pdf.cell(0, 8, to_ascii(f"Email: {student_info.get('email','')}"), ln=True)
    pdf.cell(0, 8, to_ascii(f"Date: {student_info.get('date', date.today())}"), ln=True)
    pdf.ln(2)
    completed = len(scores)
    pdf.cell(0, 8, to_ascii(f"Assignments: {assignment_total} | Completed: {completed} | Not Completed: {assignment_total-completed}"), ln=True)
    pdf.ln(4)

    # List all assignments and scores
    for _, r in scores.iterrows():
        pdf.set_font('Arial', 'B', 11)
        pdf.cell(0, 8, to_ascii(f"{r['assignment']}: {r['score']}/100"), ln=True)
        pdf.set_font('Arial', '', 10)
        pdf.multi_cell(0, 7, to_ascii(f"Comments: {str(r['comments'])}"))
        # Add reference answers if available
        ref = ref_answers.get(r['assignment'])
        if ref:
            pdf.set_font('Arial', 'I', 9)
            pdf.multi_cell(0, 6, to_ascii("Ref. Answers: " + " | ".join(ref)))
        pdf.ln(2)
    pdf.ln(4)
    ...
    for assignment, ref in ref_answers.items():
        if assignment not in done_assignments:
            pdf.set_font('Arial', 'B', 11)
            pdf.cell(0, 8, to_ascii(f"{assignment}: Not Completed"), ln=True)
            if ref:
                pdf.set_font('Arial', 'I', 9)
                pdf.multi_cell(0, 6, to_ascii("Ref. Answers: " + " | ".join(ref)))
            pdf.ln(2)
    ...
    pdf.cell(0, 10, to_ascii(f"Generated by {school_name}"), ln=True, align='R')
